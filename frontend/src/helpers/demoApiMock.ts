...
// ---------------------------------------------------------------------
const ASSET_USDC = { id: "asset-usdc", code: "USDC", issuer: "GDEMOISSUERUSDC00000000000000000000000000000000000000" };
const WALLET_VIBRANT = { id: "wallet-vibrant", name: "Vibrant Wallet", homepage: "https://vibrantapp.com" };
const WALLET_BEANS = { id: "wallet-beans", name: "Beans App", homepage: "https://www.beansapp.com" };
...
const disbursementSeed = [
  { id: "demo-disb-1", name: "March Cash Assistance - Nairobi", status: "COMPLETED", total: 1000, sent: 1000, failed: 0, canceled: 0 },
  { id: "demo-disb-2", name: "Emergency Relief - Turkana County", status: "STARTED", total: 500, sent: 340, failed: 12, canceled: 0 },
  { id: "demo-disb-3", name: "April School Fee Support", status: "READY", total: 250, sent: 0, failed: 0, canceled: 0 },
  { id: "demo-disb-4", name: "Draft - Mombasa Flood Response", status: "DRAFT", total: 0, sent: 0, failed: 0, canceled: 0 },
];
...
const mockDisbursement = (seed: (typeof disbursementSeed)[number], i: number) => ({
  ...
  receiver_registration_message_template: "You have received a payment from SAPCONE. Download the wallet to claim it.",
  ...
  file_name: seed.status === "DRAFT" ? undefined : `${seed.name.toLowerCase().replace(/\s+/g, "-")}.csv`,
});
...
const mockReceiver = (i: number) => ({
  ...
  received_amounts: [{ asset_code: "USDC", asset_issuer: ASSET_USDC.issuer, received_amount: "135.00" }],
...
const mockPayment = (i: number) => ({
  ...
  status_history: [
    ...
    { status: paymentStatuses[i % paymentStatuses.length], status_message: "", timestamp: daysAgo(4) },
  ],
...
const mockStatistics = {
  payment_counters: { canceled: 2, draft: 3, ready: 4, pending: 5, paused: 0, success: 42, failed: 3, total: 59 },
...
// ---------------------------------------------------------------------
const mockWallets = [
  { ...WALLET_VIBRANT, deep_link_schema: "vibrant://", enabled: true, assets: [ASSET_USDC], created_at: daysAgo(60), updated_at: daysAgo(1), user_managed: false },
  { ...WALLET_BEANS, deep_link_schema: "beansapp://", enabled: true, assets: [ASSET_USDC], created_at: daysAgo(60), updated_at: daysAgo(1), user_managed: false },
];

const mockAssets = [{ ...ASSET_USDC, created_at: daysAgo(90), updated_at: daysAgo(1) }];

const mockUsers = [
  { id: "demo-user-id", first_name: "Demo", last_name: "User", email: "demo@disburseflow.org", roles: ["owner"], is_active: true },
  { id: "demo-user-2", first_name: "Abigael", last_name: "Nyangasi", email: "abigael@sapcone.org", roles: ["financial_controller"], is_active: true },
  { id: "demo-user-3", first_name: "Peter", last_name: "Wesly", email: "peter@sapcone.org", roles: ["developer"], is_active: true },
];
...
export const installDemoApiMock = () => {
  ...
  console.log("%c[Demo Mode] API mock installed. API_URL =", "color: #000000; font-weight: bold;", API_URL);
  ...
  window.fetch = async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    // eslint-disable-next-line no-console
    console.log("%c[Demo Mode] intercepted", "color: #000000;", method, path);
    ...
    if (method === "GET" && path === "/organization/logo") {
      return new Response(BLANK_PNG_BYTES, { status: 200, headers: { "Content-Type": "image/png" } });
    }
    ...
    if (method === "GET" && path === "/disbursements") {
      return jsonResponse({ data: allMockDisbursements, pagination: { pages: 1, total: allMockDisbursements.length } });
    }
    ...
    if (method === "GET" && path === "/payments") {
      return jsonResponse({ data: allMockPayments, pagination: { pages: 1, total: allMockPayments.length } });
    }
    ...
    if (method === "GET" && path === "/receivers") {
      return jsonResponse({ data: allMockReceivers, pagination: { pages: 1, total: allMockReceivers.length } });
    }
